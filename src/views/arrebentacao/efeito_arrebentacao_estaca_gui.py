...
from libs.wave_pile_effect import solver_forca_onda_arrebentacao_estaca
...
def arrebentacao_estaca_gui(win):    
    ...
    # rho 3
    lbl_gamma_agua = tk.Label(app, text = '𝛾𝑤 (kN/m³)')
    lbl_gamma_agua.grid(row=3, column=0, ipadx=5, pady=5, sticky=tk.W+tk.N)
    ety_gamma_agua = tk.Entry(app, width=20)
    ety_gamma_agua.grid(row=3, column=1, padx=10, pady=5, sticky=tk.N)
    ...
    def draw_results():
        cd = float(ety_cd.get())
        gamma_agua = float(ety_gamma_agua.get())
        D = float(ety_diametro.get())
        hb = float(ety_hb.get())
        Fb = solver_forca_onda_arrebentacao_estaca(CD=cd, gamma_agua=gamma_agua, D=D, Hb=hb)
        lbl_resultado.configure(text='Fb = ' + str(round(Fb, 2)) + ' kN*m/m')
